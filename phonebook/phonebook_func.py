...
def center_windor(self, w, h): # pass in the tkinter fram(master) reference and the w and h
    # get the user's screen width amd height
    screen_width = self.master.winfo_screenwidth() # naming it screen_width
    screen_height = self.master.winfo_screnheight()
...
def addToList(self):
    ...
    if (len(var_fname) > 0 ) and (len(var_lname) > 0 ) and (len(var_phone) > 0) and(len(var_email) > 0):
        ...
        with conn:
            cursor = conn.cursor()
            cursor.execute("""SELECT COUNT(col_fullname) FROM tble_phonebook \
            ...
            if chkName == 0:
                ...
                cursor.execute("""INSERT INTO tbl_phonebook (col_fname,col_lname,col_fullname, \
                    ...
                    var_phone,var_email))
                self.listList1.insert(END, var_fullname)
                onClear(self)
            else:
...
def onDelete(self):
    var_select = self.listList1.get(self.listList1.curselection())
    conn = sqlite3.connect('db_phonebook.db')
    with conn:
        cur = conn.cursor()
        cur.execute("""SELECT COUNT(*) FROM tbl_phonebook""")
...
def onUpdate(self):
    try:
        var_select = sef.lstList1.curselection()[0]
    ...
    if (len(var_phone) > 0) and (len(var_email) > 0):
        ...
        with conn:
            ...
            if count == 0 or count2 == 0:
                response = messagebox.asokcancel("Update Request","The following changes ({}) and ({}). \
